...
class RAGPreTreatment:
    ...
    def rag_static_pre_treatment(rag:RagService, query:Union[str, Conversation]) -> tuple[QuestionTranslation, dict]:
        question_analysis, extracted_implicit_metadata, extracted_explicit_metadata = Execute.run_several_functions_as_concurrent_async_tasks(
            (RAGPreTreatment.query_rewritting_for_elearning_chatbot_async, (rag, query)),
    ...
    #TODO: TO MOVE. the query rewritting is domain specific and its prompt too (for a elearning chatbot). Therefore, it could be moved outside 'common_tools'
    @staticmethod
    @workflow_output('analysed_query')
    async def query_rewritting_for_elearning_chatbot_async(rag:RagService, analysed_query:QuestionRewritting) -> str:        
